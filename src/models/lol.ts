...
export class LoLModel {
      ...
      if (typeof summoner.id !== 'undefined') {
        ...
        promises.push(
          new Promise((resolve, reject) => {
            fetch(
              `https://euw1.api.riotgames.com/lol/champion-mastery/v4/champion-masteries/by-summoner/${pid}/?api_key=${self.apikey}`,
              {}
            )
              .then((res: any) => res.json())
              .then((body: any) => {
        ...
        promises.push(
          new Promise((resolve, reject) => {
            fetch(
              `https://euw1.api.riotgames.com/lol/league/v4/entries/by-summoner/${pid}?api_key=${self.apikey}`,
              {}
            )
              .then((res: any) => res.json())
              .then((body: any) => {
          ...
          new Promise((resolve, reject) => {
            self.getMatchlist(puuid, 1).then((body: any) => {
              ...
              self.getMatchInfo(gameId).then((body: any) => {
                ...
                match.cs = self.getCSPerMinutes(
                  participant,
                  body.info.gameDuration
                );
   ...
   */
  comparePlayers(
    options: { user1: string; user2: string },
    callback: Function
  ) {
    ...
    var self = this;
    Promise.all([
      this.getSummonerByName(user1),
      this.getSummonerByName(user2),
    ]).then((summs: any) => {
      if (
        typeof summs[0].id !== 'undefined' &&
        typeof summs[1].id !== 'undefined'
      ) {
        Promise.all([
          self.getMatchlist(summs[0].puuid, 5, 'normal'),
          self.getMatchlist(summs[1].puuid, 5, 'normal'),
        ]).then((matchlists: any) => {
            ...
            promises.push(
              self.getMatchInfo(matchlist[i]).then((game: any) => {
                ...
                return new Promise((resolve1, reject1) => {
                  ...
                  resolve1(
                    self.getCSPerMinutes(participant, game.info.gameDuration)
                  );
                });
              })
            );
  ...
  getChampionRotation(callback: Function) {
    ...
    var self = this;
    fetch(
      `https://euw1.api.riotgames.com/lol/platform/v3/champion-rotations?api_key=${this.apikey}`,
      {}
    )
      .then((res: any) => res.json())
      .then((body: any) => {
  ...
  getSummonerByName(username: string) {
    return fetch(
      `https://euw1.api.riotgames.com/lol/summoner/v4/summoners/by-name/${username}/?api_key=${this.apikey}`,
      {}
    ).then((res: any) => res.json());
  }
  ...
  getMatchlist(puuid: string, number: number, filter?: string) {
    if (filter == 'normal' || filter == 'ranked') {
      return fetch(
        `https://europe.api.riotgames.com/lol/match/v5/matches/by-puuid/${puuid}/ids?api_key=${this.apikey}&count=${number}&type=${filter}`,
        {}
      ).then((res: any) => res.json());
    } else {
      return fetch(
        `https://europe.api.riotgames.com/lol/match/v5/matches/by-puuid/${puuid}/ids?api_key=${this.apikey}&count=${number}`,
        {}
      ).then((res: any) => res.json());
    }
  }
  ...
  getMatchScore(match: any, accountId: string) {
    ...
    score.push(
      (participant.timeCCingOthers * 4) / (match.info.gameDuration / 60)
    );
    score.push(
      participant.totalDamageDealtToChampions /
        ((match.info.gameDuration * 125) / 60)
    );
    score.push(
      participant.totalDamageTaken / ((match.info.gameDuration * 150) / 60)
    );
    return score;
  }
  ...
  getMatchInfo(gameId: number) {
    return fetch(
      `https://europe.api.riotgames.com/lol/match/v5/matches/${gameId}?api_key=${this.apikey}`,
      {}
    ).then((res: any) => res.json());
  }
