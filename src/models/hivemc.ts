...
export class HivemcModel {
  ...
  getUserStats(options: { username: string }, callback: Function) {
    ...
    var hide = fetch(
      'http://api.hivemc.com/v1/player/' + username + '/HIDE',
      {}
    );
    var grav = fetch(
      'http://api.hivemc.com/v1/player/' + username + '/GRAV',
      {}
    );
    var blockparty = fetch(
      'http://api.hivemc.com/v1/player/' + username + '/BP',
      {}
    );
    var deathrun = fetch(
      'http://api.hivemc.com/v1/player/' + username + '/DR',
      {}
    );

    Promise.all([hide, grav, blockparty, deathrun])
      .then((responses) => {
        return Promise.all(
