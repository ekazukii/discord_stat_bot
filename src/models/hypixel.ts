...
class HypixelModel {
  ...
  getUserStats(options: { username: string }, callback: Function) {
    ...
    this.mojang.getUUIDByUsername(options.username, (uuid: string) => {
      fetch(
        `https://api.hypixel.net/player?uuid=${uuid}&key=${self.api_key}`,
        {}
      )
        ...
        .then((body: any) => {
          if (typeof body.player !== 'undefined' && body.player !== null) {
            ...
            if (typeof body.player.stats['Bedwars'] !== 'undefined') {
              response.wins.push(
                body.player.stats['Bedwars'].wins_bedwars || 0
              );
            } else {
              response.wins.push(0);
            }
            ...
            response.plevel = Math.round(
              Math.sqrt(2 * body.player.networkExp + 30625) / 50 - 2.5
            );
            ...
            promises.push(
              new Promise((resolve, reject) => {
                fetch(
                  `https://api.hypixel.net/status?uuid=${uuid}&key=${self.api_key}`,
                  {}
                )
                  .then((res: any) => res.json())
                  .then((body: any) => {
            ...
            promises.push(
              new Promise((resolve, reject) => {
                fetch(
                  `https://api.hypixel.net/findGuild?byUuid=${uuid}&key=${self.api_key}`,
                  {}
                )
                  .then((res: any) => res.json())
                  .then((body: any) => {
                    if (
                      typeof body.guild !== 'undefined' &&
                      body.guild !== null
                    ) {
                      fetch(
                        `https://api.hypixel.net/guild?id=${body.guild}&key=${self.api_key}`,
                        {}
                      )
                        .then((res: any) => res.json())
                        .then((body1: any) => {
